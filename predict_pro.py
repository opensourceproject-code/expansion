...
def select_database(sql):
    # connect database
    connect = pymysql.connect(user="root",password="123456",host="127.0.0.1",port=3306,db="english_data1",charset="utf8")
...
def text_match1(attribute_list,answer_list,sentence):
...
def main():
    ...
    with torch.no_grad():
        ...
        tokenizer_kwards = {'do_lower_case': False,
                            'max_len': 64,
                           # 'vocab_file': './pytorch_bert/bert-base-english-vocab.txt'
        ...
        ner_model = get_ner_model(config_file = #'./pytorch_bert/bert-base-english-config.json'
                                  './input/config/bert-base-chinese-config.json',
                                  pre_train_model = './output3/best_ner.bin',label_num = len(ner_processor.get_labels()))
        ner_model = ner_model.to(device)
        ner_model.eval()

        sim_model = get_sim_model(config_file=#'./pytorch_bert/bert-base-english-config.json'
                                  './input/config/bert-base-chinese-config.json',
                                  pre_train_model='./output3/best_sim.bin',
        ...
        while var == 1:
            ...
            line1 = []
            f = open("toExpansion.txt", "r")
            f1 = open("EnscriptAbbreviation.txt", "r")
            f2 = open("write.txt","w")
            lines = f1.readlines()
            for line in lines:
            ...
            for line in f:
                ...
                for i in range(line.strip().count(' ') + 1):
                    if line.strip().split(' ')[i] in line1:
                        print("=====" * 10)
                        raw_text = line.strip().split(' ')[i] + '的扩展是什么？'
                        ...
                        if '' == entity:
                            print("未发现实体")
                            continue
                        sql_str = "select * from wycheproof where entity = '{}'".format(entity)
                        ...
                        if(len(triple_list[2]) <= 1):
                            ...
                            if attribute != '' and answer != '':
                                ...
                                sim_model = sim_model.to(device)
                                sim_model.eval()
                                attribute_idx = semantic_matching(sim_model, tokenizer, raw_text, attribute_list,
                                                                  answer_list, 64).item()
                                if -1 == attribute_idx:
                                    ret = ''
                                else:
                            ...
                            print("请添加补充信息：")
                            plus = input();
                            attribute, answer = text_match1(attribute_list, answer_list, plus)
                            if attribute != '' and answer != '':
                                # ret = "{}的{}是{}".format(entity, attribute, answer)
                                ret = "{}".format(answer)
                            else:
